...
export default function TeacherLessonDetail() {
  ...
  const handleSaveLesson = async () => {
    if (!lesson || !id) return;
    try {
      // Transform lesson data to match API expectations
      const lessonData = {
        ...lesson,
        vocabularies: lesson.vocabularies.map(v => ({
          ...v,
          id: v.id || 0, // Convert null to 0 for new items
        })),
        grammars: lesson.grammars.map(g => ({
          ...g,
          id: g.id || 0, // Convert null to 0 for new items
        })),
      };
      ...
      console.error('Save lesson error:', err); // Debug log
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Bài học: {lesson.title}</h1>
      </div>
