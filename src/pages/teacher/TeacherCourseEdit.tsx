...
export const TeacherCourseEdit: React.FC = () => {
  ...
  useEffect(() => {
    // Initialize form with mock data since backend detail endpoint is not available
    const mockCourse = {
      id: Number(id),
      ...
      level: 'beginner',
      duration: '8 tuần',
      objectives: ['Học cách chào hỏi cơ bản', 'Hiểu ngữ pháp cơ bản'],
    ...
    setFormData({
      ...
      categoryId: mockCourse.categoryId.toString(),
  ...
  const handleSubmit = async (status?: Course['status']) => {
    ...
    // Course editing is not yet supported by the backend
    alert('Tính năng chỉnh sửa khóa học hiện chưa được hỗ trợ bởi hệ thống backend. Vui lòng thử lại sau.');
    
    // TODO: Implement when backend supports course editing
    console.log('Course edit attempted:', {
      id: Number(id),
  ...
  const handleDelete = async () => {
    // Course deletion is not yet supported by the backend
    alert('Tính năng xóa khóa học hiện chưa được hỗ trợ bởi hệ thống backend. Vui lòng thử lại sau.');
    
    // TODO: Implement when backend supports course deletion
    console.log('Course deletion attempted:', id);
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Button
            variant="outline"
            size="sm"
            onClick={() => navigate(ROUTES.TEACHER.COURSES.DETAIL.replace(':id', id!))}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <Select value={formData.status} onValueChange={(value) => handleInputChange('status', value as Course['status'])}>
                <SelectTrigger>
                ...
                <div className="flex justify-between">
                  ...
                  <Badge variant={formData.status === 'active' ? 'default' : 'secondary'}>
                    {formData.status === 'active' ? 'Đang hoạt động' : 
                     formData.status === 'draft' ? 'Bản nháp' : 'Đã lưu trữ'}
                  </Badge>
                </div>
                <div className="flex justify-between">
